...
namespace multigrid_detail {
    ...
    template <typename T>
    void damped_jacobi_smooth(const MatrixXX<T> &A,
                              const VectorX<T> &b,
                              VectorX<T> &x,
                              T omega,
                              index_type num_sweeps) {
...
auto vcycle(const std::vector<MultigridLevel<T>> &hierarchy,
            ...
            VectorX<T> &x,
            T omega,
            index_type pre_sweeps,
            index_type post_sweeps) -> bool {
...
auto multigrid(const std::vector<MultigridLevel<T>> &hierarchy,
               ...
               const VectorX<T> &x0,
               T tol = T{1e-10},
               index_type max_iter = 100,
               T omega = T{2} / T{3},
               index_type pre_sweeps = 2,
               index_type post_sweeps = 2) {
    using Vec = VectorX<T>;
    ...
    for (index_type iter = 0; iter < max_iter; ++iter) {
        ...
        if (rnorm <= tol) {
            return std::expected<Result, SolverError>{Result{
                .x = std::move(x), .residual_norm = rnorm, .iterations = iter}};
        }
    ...
    if (rnorm <= tol) {
        return std::expected<Result, SolverError>{Result{
            .x = std::move(x), .residual_norm = rnorm, .iterations = max_iter}};
    }
...
auto multigrid(const std::vector<MultigridLevel<T>> &hierarchy,
    ...
    return multigrid(
        hierarchy, b, x0, tol, max_iter, omega, pre_sweeps, post_sweeps);
}
