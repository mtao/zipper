...
///
/// Creates a unit quaternion from angles in radians as (pitch, yaw, roll)
/// corresponding to rotations about the X, Y, Z axes respectively.
/// Uses the intrinsic Tait-Bryan convention (ZYX order):
///   q = qZ * qY * qX
///
/// This is the inverse of euler_angles().
///
/// @param angles Vector of (pitch, yaw, roll) in radians.
/// @return       Unit quaternion representing the rotation.
template <zipper::concepts::Vector V>
  requires(V::extents_type::static_extent(0) == 3)
auto from_euler_angles(const V &angles) -> Quaternion<typename V::value_type> {
  using T = typename V::value_type;
  T pitch = angles(0); // X
  T yaw = angles(1);   // Y
  T roll = angles(2);  // Z

  T cx = std::cos(pitch * T(0.5)), sx = std::sin(pitch * T(0.5));
  T cy = std::cos(yaw * T(0.5)), sy = std::sin(yaw * T(0.5));
  T cz = std::cos(roll * T(0.5)), sz = std::sin(roll * T(0.5));

  return Quaternion<T>(cx * cy * cz + sx * sy * sz,  // w
                       sx * cy * cz - cx * sy * sz,  // x
                       cx * sy * cz + sx * cy * sz,  // y
                       cx * cy * sz - sx * sy * cz); // z
}
...
auto to_quaternion(const M &m) -> Quaternion<typename M::value_type> {
  ...
  if (trace > T(0)) {
    ...
    return Quaternion<T>(T(0.25) / s, (m(2, 1) - m(1, 2)) * s,
                         (m(0, 2) - m(2, 0)) * s, (m(1, 0) - m(0, 1)) * s);
  } else if (m(0, 0) > m(1, 1) && m(0, 0) > m(2, 2)) {
    T s = T(2) * std::sqrt(T(1) + m(0, 0) - m(1, 1) - m(2, 2));
    return Quaternion<T>((m(2, 1) - m(1, 2)) / s, T(0.25) * s,
                         (m(0, 1) + m(1, 0)) / s, (m(0, 2) + m(2, 0)) / s);
  } else if (m(1, 1) > m(2, 2)) {
    T s = T(2) * std::sqrt(T(1) + m(1, 1) - m(0, 0) - m(2, 2));
    return Quaternion<T>((m(0, 2) - m(2, 0)) / s, (m(0, 1) + m(1, 0)) / s,
                         T(0.25) * s, (m(1, 2) + m(2, 1)) / s);
  } else {
    T s = T(2) * std::sqrt(T(1) + m(2, 2) - m(0, 0) - m(1, 1));
    return Quaternion<T>((m(1, 0) - m(0, 1)) / s, (m(0, 2) + m(2, 0)) / s,
...
auto slerp(const Q1 &q1, const Q2 &q2, typename Q1::value_type t)
    -> Quaternion<typename Q1::value_type> {
  ...
  if (cosTheta > T(1) - std::numeric_limits<T>::epsilon()) {
    return Quaternion<T>(
        q1.w() + t * (q2a.w() - q1.w()), q1.x() + t * (q2a.x() - q1.x()),
        q1.y() + t * (q2a.y() - q1.y()), q1.z() + t * (q2a.z() - q1.z()));
  }
  ...
  return Quaternion<T>(w1 * q1.w() + w2 * q2a.w(), w1 * q1.x() + w2 * q2a.x(),
                       w1 * q1.y() + w2 * q2a.y(), w1 * q1.z() + w2 * q2a.z());
}
