...
#include "zipper/storage/detail/SparseCompressedAssign.hpp"
...
class SparseCompressedAccessor
  : public expression::ExpressionBase<SparseCompressedAccessor<ValueType,
                                                               Extents,
                                                               LayoutPolicy,
                                                               StoragePolicy>>
  , public Extents {
  public:
    using ParentType =
        expression::ExpressionBase<SparseCompressedAccessor<ValueType,
                                                            Extents,
                                                            LayoutPolicy,
                                                            StoragePolicy>>;
    ...
    using compressed_data_type = detail::SparseCompressedData<
    ...
    constexpr static bool is_csr = std::is_same_v<LayoutPolicy, layout_right>;
    constexpr static bool is_csc = std::is_same_v<LayoutPolicy, layout_left>;
    static_assert(
        rank() < 2 || is_csr || is_csc,
        "LayoutPolicy must be layout_right (CSR) or layout_left (CSC) "
        "for rank >= 2");

    // For rank-2: which user-facing dimension is the outer/inner compressed
    // dim. For rank-1: defaulted to 0, but never meaningfully used.
    ...
    /// Assign from an arbitrary expression.
    ///
    /// Dispatch order (first match wins):
    ///   1. Same-layout compressed → direct data copy (O(nnz))
    ///   2. Sparse matmul (MatrixProduct<SCA, SCA>) → Gustavson SpGEMM
    ///   3. Sparse add/sub (ZeroAwareOperation<SCA, SCA, +/->) → sorted merge
    ///   4. Sparse transpose (Swizzle<SCA, 1, 0>) → count-scatter
    ///   5. Generic fallback → COO intermediary
    ///
    /// Only available for owned storage (span views are structurally
    /// immutable).
    template <zipper::concepts::Expression V>
    void assign(const V &v)
        requires(!std::is_const_v<ValueType> && !is_span
                 && zipper::utils::extents::assignable_extents_v<
                     typename V::extents_type,
                     extents_type>)
    {
        // Fast path 1: same-layout compressed → compressed copy
        ...
        // Fast path 2: SpGEMM — MatrixProduct<SCA, SCA>
        else if constexpr (detail::is_sparse_matmul_v<V, LayoutPolicy>) {
            if constexpr (!IsStatic) {
                static_cast<extents_type &>(*this) =
                    zipper::detail::ExtentsTraits<extents_type>::convert_from(
                        v.extents());
            }
            detail::SparseCompressedAssign::assign_spgemm(m_data, v);
            return;
        }
        // Fast path 3: Sparse add/sub — ZeroAwareOperation<SCA, SCA, +/->
        else if constexpr (detail::is_sparse_add_v<V, LayoutPolicy>) {
            if constexpr (!IsStatic) {
                static_cast<extents_type &>(*this) =
                    zipper::detail::ExtentsTraits<extents_type>::convert_from(
                        v.extents());
            }
            detail::SparseCompressedAssign::assign_add(m_data, v);
            return;
        }
        // Fast path 4: Sparse transpose — Swizzle<SCA, 1, 0>
        else if constexpr (detail::is_sparse_transpose_v<V, LayoutPolicy>) {
            if constexpr (!IsStatic) {
                static_cast<extents_type &>(*this) =
                    zipper::detail::ExtentsTraits<extents_type>::convert_from(
                        v.extents());
            }
            detail::SparseCompressedAssign::assign_transpose(m_data, v);
            return;
        }
        // Fallback: generic COO intermediary path
        else {
            auto make_coo = [&]() {
                if constexpr (IsStatic) {
                    return SparseCoordinateAccessor<element_type,
                                                    extents_type>();
                } else {
                    return SparseCoordinateAccessor<element_type, extents_type>(
                        zipper::detail::ExtentsTraits<
                            extents_type>::convert_from(v.extents()));
                }
            };
            auto coo = make_coo();
    ...
    // ── index_set: rank-2 ────────────────────────────────────────────────
    // For CSR: D==1 (cols for given row) is fast O(nnz_row), D==0 is slow
    // O(nnz). For CSC: D==0 (rows for given col) is fast O(nnz_col), D==1 is
    // slow O(nnz). Generalized: D==inner_dim is fast (indptr lookup),
    // D==outer_dim is slow.
    template <rank_type D>
    ...
    auto index_set(index_type other_idx) const
        ...
        if constexpr (D == inner_dim) {
            ...
            if (other_idx >= m_data.outer_size()) { return {{}}; }
            auto start = m_data.m_indptr[other_idx];
            auto end = m_data.m_indptr[other_idx + 1];
            using Base = detail::SparseCompressedData<
                typename compressed_data_type::value_type,
                0,
                StoragePolicy>;
            ...
            for (index_type outer = 0; outer < m_data.outer_size(); ++outer) {
                ...
                auto it = std::lower_bound(base.m_indices.begin() + start,
                                           base.m_indices.begin() + end,
                                           other_idx);
...
SparseCompressedAccessor<ValueType, Extents, LayoutPolicy, StoragePolicy>::
    ~SparseCompressedAccessor() = default;
...
struct detail::ExpressionTraits<
    zipper::storage::SparseCompressedAccessor<ValueType,
                                              Extents,
                                              LayoutPolicy,
                                              StoragePolicy>>
