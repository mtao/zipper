...
namespace detail {
    ...
    struct DefaultUnaryExpressionDetail {
        using child_traits =
            expression::detail::ExpressionTraits<std::decay_t<Child>>;
        using base_value_type = typename child_traits::value_type;
    };

    /// Default traits for unary expressions. Inherits from
    /// BasicExpressionTraits to provide the access_features/shape_features
    /// interface.
    ///
    /// Child is the *qualified* child type as passed to the unary expression
    /// class template (e.g. `const MDArray<…>&` for reference storage,
    ...
    struct DefaultUnaryExpressionTraits
      : public expression::detail::BasicExpressionTraits<
        ...
        constexpr static bool stores_references =
            std::is_reference_v<Child>
            || _Detail::child_traits::stores_references;
        ...
        using preferred_layout =
            typename _Detail::child_traits::preferred_layout;
    };
} // namespace detail
...
class UnaryExpressionBase : public expression::ExpressionBase<Derived> {
  public:
    ...
    using child_type = std::
        conditional_t<is_assignable, expression_type, const expression_type>;
