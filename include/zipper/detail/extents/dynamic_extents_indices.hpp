...
struct DynamicExtentIndices<zipper::extents<indices...>> {
    ...
    // for an index J, if E::static_extent(J) == std::dynamic_extent returns the index of that extent
    ...
    consteval static const dynamic_indices_type get_dynamic_local_indices(
        std::index_sequence<Indices...>) {
        return std::array<rank_type, dynamic_rank>{
            {get_dynamic_local_index<Indices>()...}};
    }

    static consteval dynamic_indices_type get_dynamic_local_indices() {
