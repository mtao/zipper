...
class CSMatrix
  ...
  public:
    ...
    auto as_span() -> span_type {
        ...
        using SpanData =
            storage::detail::SparseCompressedSpanData<value_type, 1>;
        SpanData span_data(
            std::span<const index_type>(cd.m_indptr.data(), cd.m_indptr.size()),
            std::span<const index_type>(cd.m_indices.data(),
                                        cd.m_indices.size()),
    ...
    auto as_const_span() const -> const_span_type {
        ...
        SpanData span_data(
            ...
            std::span<const index_type>(cd.m_indices.data(),
                                        cd.m_indices.size()),
            std::span<const value_type>(cd.m_values.data(),
                                        cd.m_values.size()));
    ...
    auto to_coo() const -> COOMatrix<value_type, Rows, Cols> {
        auto make_coo = [this]() -> COOMatrix<value_type, Rows, Cols> {
            if constexpr (is_static) {
                return COOMatrix<value_type, Rows, Cols>{};
            } else {
                return COOMatrix<value_type, Rows, Cols>(extents());
            }
        };
        auto result = make_coo();
        const auto &cd = compressed_data();
        using Base0 = storage::detail::
            SparseCompressedData<std::remove_const_t<value_type>, 0>;
    ...
    auto as_csr() const
        -> CSMatrix<value_type, Rows, Cols, storage::layout_right> {
        if constexpr (is_csr) {
            return *this;
        } else {
            return CSMatrix<value_type, Rows, Cols, storage::layout_right>(
                to_coo());
        }
    }
    ...
    auto as_csc() const
        -> CSMatrix<value_type, Rows, Cols, storage::layout_left> {
        if constexpr (is_csc) {
            return *this;
        } else {
            return CSMatrix<value_type, Rows, Cols, storage::layout_left>(
                to_coo());
        }
    }
};
