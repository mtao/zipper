...
namespace detail {
template <typename T> struct is_slice : public std::false_type {};
...
template <typename T> struct slice_like : public index_like<T> {};

template <IndexLike OffsetType, IndexLike ExtentType, IndexLike StrideType>
struct slice_like<slice_type<OffsetType, ExtentType, StrideType>>
    : public std::true_type {};

template <> struct slice_like<full_extent_t> : public std::true_type {};
...
template <zipper::concepts::View View>
  requires(View::extents_type::rank() == 1)
struct slice_like<View> : public std::true_type {};

} // namespace detail

// A value is slice-like if it can belong in a slice.
