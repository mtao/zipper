...
TEST_CASE("format_matrix", "[format]") {
    ...
    CHECK(std::format("{}", x) == "[[0, -3, 100];[1, 3, 4];[2, 0, 1]]");
}
...
TEST_CASE("format_matrix_transpose", "[format]") {
    ...
    CHECK(std::format("{}", M.transpose()) == "[[1, 4];[2, 5];[3, 6]]");
}
...
TEST_CASE("format_form", "[format]") {
    ...
    CHECK(std::format("{}", f) == "[100, 200, 300]");
}
...
TEST_CASE("format_tensor_2d", "[format]") {
    ...
    CHECK(std::format("{}", t) == "[[1, 2, 3];[4, 5, 6]]");
}
