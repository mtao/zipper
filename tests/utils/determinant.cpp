...
TEST_CASE("test_matrix_determinant", "[utils][determinant]") {
    REQUIRE(zipper::utils::extents::detail::max_dim<zipper::extents<3, 4>>() ==
            4);
    REQUIRE(zipper::utils::extents::detail::max_dim<zipper::extents<2, 4>>() ==
            4);
    REQUIRE(zipper::utils::extents::detail::max_dim<zipper::extents<4, 2>>() ==
            4);
    // static assert preents these
    // REQUIRE(zipper::utils::extents::detail::max_dim<
    ...
    CHECK(zipper::utils::extents::is_cubic(zipper::extents<4, 4>{}));
    CHECK_FALSE(zipper::utils::extents::is_cubic(zipper::extents<3, 4>{}));
    CHECK_FALSE(zipper::utils::extents::is_cubic(zipper::extents<5, 4>{}));

    CHECK(zipper::utils::extents::is_cubic(
        zipper::extents<std::dynamic_extent, 4>{4}));
    CHECK_FALSE(zipper::utils::extents::is_cubic(
